 ...
 */
const DebugFill = ({ name }) => (react_1.default.createElement(Debug, { "data-slot-name": name },
    "Slot: \"",
    name,
    "\""));
/**
...
const useFills = (name) => {
    var _a;
    const { state, libraries } = __1.useConnect();
    if (!name) {
        __1.warn("You should pass the name of the slot that you would like to fill!");
        return [];
    }
    ...
    Object.entries(state.fills || {})
        ...
        ([key, fill]) => (Object.assign(Object.assign({ key: `${namespace} - ${key}`, priority: fill.priority || 10 }, (fill.library && {
            Fill: get_value_1.default(libraries.fills, fill.library),
        })), fill))),
    ], [])
        ...
        if (!Fill)
            __1.warn(`The Fill component for "${key}" cannot be found in "libraries.fills.${library}"`);
        return !!Fill;
    })
        // Sort by priority
